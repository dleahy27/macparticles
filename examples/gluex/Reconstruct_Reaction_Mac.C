...
struct HistsPhys{
  TH1D hM2pi={"M2pi","Invariant Mass #pi+#pi- (GeV)",110,0,3.6};
  TH1D hW={"W","W (GeV)",110,1,8};
  TH1D hCosTh={"CosTh","cos(#theta_{GJ})",110,-1,1};
  TH1D hPhi={"Phi","#phi_{GJ} (#circ)",110,-180,180};
  TH1D hCMCosTh={"CosTh","cos(#theta_{CM})",110,-1,-0.7};
...
struct HistsPhysRes{
  ...
  TH1D hpPhi={"pPhi","proton #phi Resolution (#circ)",100,-360,360};
  TH1D hpimP={"pimP","#pi- Momentum Resolution (GeV)",100,-10,10};
  TH1D hpimTheta={"pimTheta","#pi- #theta Resolution(#circ)",100,-45,45};
  TH1D hpimPhi={"pimPhi","#pi- #phi Resolution (#circ)",100,-360,360};
  TH1D hpipP={"pipP","#pi+ Momentum Resolution (GeV)",100,-10,10};
  TH1D hpipTheta={"pipTheta","#pi+ #theta Resolution (#circ)",100,-45,45};
  TH1D hpipPhi={"pipPhi","#pi+ #phi Resolution (#circ)",100,-360,360};
  void Draw(){
    auto can=new TCanvas;
    can->Divide(3,2);
...
void Reconstruct_Reaction_Mac(){
  ...
  TTree          *fChain = 0;   //!pointer to the analyzed TTree or TChain
   ...
   TH2D rpiDiff("rpi","pi",100,-12,12,100,-12,12);
   TH2D fpiDiff("fpi","pi",100,-12,12,100,-12,12);
   ...
   while(fReader.Next()){
     ...
     if(*accepted_p&&*fast_acc_p){
       hf.Diff_pP.Fill(*rec_pP-*fast_pP);
     }
     if(*accepted_pip){
